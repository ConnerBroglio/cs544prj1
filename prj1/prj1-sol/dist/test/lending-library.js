...
import { BOOKS, LANG_BOOKS, BOOK_nCopies2 } from './data/data.js';
    ...
    describe('findBooks()', () => {
        ...
        it('must find single results', () => {
            for (const lang of ['javascript', 'ruby', 'scala']) {
                ...
                const expected = [{ nCopies: 1, ...LANG_BOOKS[lang] }];
        ...
        it('must find intersection of results', () => {
            const searchResult = library.findBooks({ search: 'a #definitive @JAVASCRIPT' });
            assert(searchResult.isOk === true);
            expect(searchResult.val).to.have.length(1);
